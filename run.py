...
def getcmd(host, blks, test_time, stream_count, reverse, protocol, iperffile):
    ...
    if platform.system() == "Windows":
        cmd += ".exe"
    if protocol == "tcp":
        cmd += " -c " + host + " -J -M " + str(blks) + " -t " + str(test_time)
        if not reverse:
            cmd += " --get-server-output"
    else:
        cmd += (
            " -c " + host + " -J -t " + str(test_time) + " -u -b 100G -l " + str(blks)
        )
        if not reverse:
            cmd += " --get-server-output"
    if stream_count > 1:
...
def runtcp(filename, data, blks, cmd, reverse):
    ...
    res = round(float((storedata["end"]["sum_received"]["bits_per_second"]) / 1000000))
...
def runudp(filename, data, blks, cmd, reverse, test_time):
    ...
    if reverse:
        for timetest in timerenge:
            res = round(
                float(
                    (storedata["intervals"][timetest]["sum"]["bits_per_second"])
                    / 1000000
                )
            )
            list_res.append(res)
        ...
        for timetest in timerenge:
            res = round(
                float(
                    (
                        storedata["server_output_json"]["intervals"][timetest]["sum"][
                            "bits_per_second"
                        ]
                    )
                    / 1000000
            ...
            list_res.append(res)
        procentlost = round(
            float(storedata["server_output_json"]["end"]["sum"]["bits_per_second"])
        )
        jitter_ms = float(storedata["server_output_json"]["end"]["sum"]["jitter_ms"])
        lost_packets = round(
            float(storedata["server_output_json"]["end"]["sum"]["lost_packets"])
        )
        sent_packets = round(
            float(storedata["server_output_json"]["end"]["sum"]["packets"])
        )
        tr_packets = round((sent_packets - lost_packets) / test_time)
    res = round((sum(list_res)) / test_time)
    add_line(filename, blks, res)
    proceedresultudp(reverse, res, blks, data, procentlost, jitter_ms, tr_packets)
...
def proceedresultudp(reverse, res, blks, data, procentlost, jitter_ms, tr_packets):
    """Record udp results into data."""
    if reverse:
        print(
            "Download Mbps:",
            res,
            "Packet Lost in procent:",
            procentlost,
            "jitter_ms:",
            jitter_ms,
            "Packets transmitted",
...
def graph(png_filename, x_1, y_1, x_2, y_2, ax_1, ay_1, ax_2, ay_2, title, max_y):
...
def argparse_hostname_or_ip(value):
    ...
    allowed = re.compile(r"(?!-)[a-z0-9\.\:\[\]-]{1,253}(?<!-)$", re.IGNORECASE)
...
def get_options(args):
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            """Set custom name for iperf file, file should be in directory
    ...
    if options.range_from > options.range_to:
        print(
            ...
            % (str(options.range_from), str(options.range_to))
        )
        sys.exit(-1)
    return options
...
def main():
    ...
    psize = list_range(
        options.range_from, options.range_to, options.interval, options.list_mss
    )
    ...
    cmd = getcmd(host, psize, options.test_time, stream_count, True, prot, iperffile)
    print("", cmd)
    add_line(filename, "command for run iperf", cmd)
    add_line(filename, "Download", "traffic")
    for blks in psize:
        listary = 0
        cmd = getcmd(host, blks, options.test_time, stream_count, True, prot, iperffile)
    ...
    cmd = getcmd(host, psize, options.test_time, stream_count, False, prot, iperffile)
    print("\n", cmd)
    add_line(filename, "Command for run iperf", cmd)
    ...
    for blks in psize:
        listaty = 0
        cmd = getcmd(
            host, blks, options.test_time, stream_count, False, prot, iperffile
        )
        for _again in range(options.repeat):
            if prot == "udp":
    ...
    max_y = max_y_result(
        data["listry"], data["listty"], data["listary"], data["listaty"]
    )
